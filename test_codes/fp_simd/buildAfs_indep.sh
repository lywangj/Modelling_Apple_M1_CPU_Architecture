...
OPArray=("fadd_s_D_indep" "bic_v_reg_16B_indep" "fmadd_s_D_indep" "fmul_v_2D_indep" "fdiv_v_2D_indep" "fdiv_s_2D_indep" "frsqrte_s_S_indep" "frsqrte_v_2D_indep")
REPEAT=10
...
test_insn () {
  ...
  elif [ $1 == "fadd_s_D_indep" ]
  then
    echo "        fadd d0, d1, d1" >> $DIR/$1_$2.s
  elif [ $1 == "fsub_v_4H" ]
  then
  ...
  elif [ $1 == "bic_v_reg_16B_indep" ]
  then
    echo "        bic v0.16b, v1.16b, v1.16b" >> $DIR/$1_$2.s
  elif [ $1 == "fcmeq_v_4H" ]
  then
  ...
  elif [ $1 == "fmadd_s_D_indep" ]
  then
    echo "        fmadd d0, d1, d1, d2" >> $DIR/$1_$2.s
  elif [ $1 == "fmul_v_2D" ]
  then
    echo "        fmul v0.2d, v0.2d, v1.2d" >> $DIR/$1_$2.s
  elif [ $1 == "fmul_v_2D_indep" ]
  then
    echo "        fmul v0.2d, v1.2d, v1.2d" >> $DIR/$1_$2.s
  elif [ $1 == "mul_v_8B" ]
  then
  ...
  elif [ $1 == "fdiv_v_2D_indep" ]
  then
    echo "        fdiv v0.2d, v1.2d, v1.2d" >> $DIR/$1_$2.s
  elif [ $1 == "fdiv_s_2D" ]
  then
    echo "        fdiv d0, d0, d1" >> $DIR/$1_$2.s
  elif [ $1 == "fdiv_s_2D_indep" ]
  then
    echo "        fdiv d0, d1, d1" >> $DIR/$1_$2.s
  elif [ $1 == "fdiv_v_8H" ]
  then
  ...
  elif [ $1 == "frsqrte_s_S_indep" ]
  then
    echo "        frsqrte s0, s1" >> $DIR/$1_$2.s
  elif [ $1 == "frsqrte_v_2D" ]
  then
    echo "        frsqrte v0.2d, v0.2d" >> $DIR/$1_$2.s
  elif [ $1 == "frsqrte_v_2D_indep" ]
  then
    echo "        frsqrte v0.2d, v1.2d" >> $DIR/$1_$2.s
  else
    continue
  fi
}
...
for ((i = 1; i <= $REPEAT; i++)); do
  for f in $DIR/*; do
    if [ -f "$f" ] && [ -x "$f" ]; then
      ...
      sudo taskset -c 4 perf stat -C 4 --all-user -e cycles,instructions "$f" &>> $DIR/perf_results.txt
    fi
  done
done
...
for f in $DIR/*; do
  if [ -f "$f" ] && [ -x "$f" ]; then
    ...
    /home/liyun/project/install/bin/simeng /home/liyun/project/configs/m1fs_v1p3.yaml $f >> $DIR/sim_results.txt
  fi
done
